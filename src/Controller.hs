...
module Controller where
  ...
  spaceBar secs gstate 
      | S.member (SpecialKey KeySpace) (keys gstate) && bulletCounter speler > 0.2 = gstate {bullets = shoot speler gstate, player = gplayer speler }
      | otherwise                                    = gstate {player = secplayer speler}
        where speler = player gstate
              gplayer p = p {bulletCounter = 0}
              secplayer p = p {bulletCounter = (bulletCounter p) + secs}
